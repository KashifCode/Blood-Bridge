...
const UserData = () => {
  ...
  return (
    <>
      ...
      {changeProfilePic ? (
        <>
          <h1 className="text-lg font-RobotoBold pt-1">
            Blood Bank Management
          </h1>
          <ChevronLeft
            size={22}
            className="mt-1.5 cursor-pointer relative -left-1.5"
            onClick={() => setChangeProfilePic(false)}
          />
          <div className="w-full flex flex-col items-center gap-y-2 my-1.5">
            <Image
              src={bloodBank?.avatar}
              alt="Profile"
              className="!w-[186px] !h-[186px] object-cover rounded-full"
              width={186}
              height={186}
            />
            ...
            <p className="text text-slate-950 text-opacity-80 font-LatoRegular">
              To upload photo, click on Upload
            </p>
            <Button
              type="submit"
              className="!w-max !rounded-[5px] !bg-darkRed hover:!bg-red-800 mt-1.5 !h-auto !py-1.5 min-w-[80px] !text-sm mb-2"
        ...
        <>
          <div className="relative">
            {!changeDetails && (
              <span
                className="absolute top-10 left-0 w-max rounded-3xl bg-darkRed hover:bg-red-800 px-4 py-0.5 text-sm text-white text-center cursor-pointer"
          ...
          <div className="w-full min-h-[55vh] pr-2 pt-1 pb-3">
            <div className="flex items-center gap-x-1.5">
              <Switch
                checked={bloodBank?.status === "open"}
                onCheckedChange={changeActiveStatus}
                id="ativeStatus"
            ...
            <Accordion type="single" collapsible>
              <AccordionItem
                value="Profile"
                className="!border-b-2 !border-slate-950 !border-opacity-60"
              >
                <AccordionTrigger className="hover:!no-underline !py-2">
                  <div className="flex flex-col items-start">
                    <h1 className="font-RobotoBold pt-1">Profile</h1>
                    <p className="text-sm text-slate-950 text-opacity-50 font-LatoRegular">
                      Name, Email Address
                    </p>
                  </div>
                </AccordionTrigger>
                <AccordionContent>
                  <form onSubmit={handleSubmit(SubmitData)}>
                    <div className="flex flex-col gap-y-2.5">
                      {changeDetails ? (
                        <input
                          ...
                          className="bg-white rounded-[5px] w-full py-2 px-3.5 focus:outline-0"
                          type="text"
                        />
                      ) : (
                        <input
                    ...
                    {changeDetails && (
                      <Button
                        type="submit"
                        className="!w-max !rounded-3xl !bg-darkRed hover:!bg-red-800 mt-3 !h-auto !py-1.5 min-w-[110px] !text-sm"
                      >
                        Update
                      </Button>
                    )}
              ...
              >
                <AccordionTrigger className="hover:!no-underline !py-2">
                  <div className="flex flex-col items-start">
                    ...
                    <p className="text text-slate-950 text-opacity-50 font-LatoRegular">
                      * * * * * * * * * * *
                    </p>
                  </div>
                </AccordionTrigger>
              ...
              >
                <AccordionTrigger className="hover:!no-underline !py-2">
                  <div className="flex flex-col items-start">
                    ...
                    <p className="text text-slate-950 text-opacity-50 font-LatoRegular">
                      if you no longer want to use this account
                    </p>
                  </div>
                </AccordionTrigger>
                <AccordionContent>
