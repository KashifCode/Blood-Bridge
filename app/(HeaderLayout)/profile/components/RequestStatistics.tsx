...
const RequestStatistics = ({
    ...
    axios
      ...
      .then((res) => {
        if (
          ...
          return setBloodRequests(res.data.bloodRequests);
        if (selectedMonth === "all")
          return setBloodRequests(
            res.data.bloodRequests.filter(
              (request: any) => request.bloodGroup.bloodGroup === selectedType,
            ),
          );
        if (selectedType === "All" || selectedType === "all")
          return setBloodRequests(
            res.data.bloodRequests.filter(
              (request: any) =>
                new Date(request.bloodNeededOn.split("T")[0])
          ...
          res.data.bloodRequests.filter(
            (request: any) =>
              new Date(request.bloodNeededOn.split("T")[0])
                .toLocaleString("default", { month: "short" })
                .toLowerCase() === selectedMonth &&
              request.bloodGroup.bloodGroup === selectedType,
          ),
        );
      })
      .catch((err) => {
  ...
  return (
    <div
      className={cx(
        `w-full bg-stone-50 pl-[2%] pr-[4%] pt-6 relative h-full`,
        { "overflow-hidden !pb-0": showModal },
      ...
      <div className="w-full h-full pb-5">
        ...
        >
          <div className="w-1/2 mx-auto h-[50vh] bg-white rounded-2xl relative pt-10 px-4">
            ...
            <p className="font-DMSansRegular text-black">
              Select Time for {bloodRequests?.[updateStatus?.index!]?.name} to
            ...
            <div className="flex flex-col gap-y-4 mt-3">
              <div className="flex flex-col gap-y-1">
                ...
                <input
                  ...
                  value={
                    bloodRequests?.[updateStatus?.index!]?.bloodNeededOn.split(
                      "T",
                    )[0]
                  }
                  readOnly
          ...
          <div className="w-full flex gap-x-7 items-center">
              ...
              <h3 className="font-PlayfairDisplayBold capitalize text-white text-lg leading-5">
                {
                  bloodRequests?.filter(
                    (request) => request.reqStatus === "Completed",
                  ).length
                }
              </h3>
            </div>
            <div className="py-3 px-4 rounded-xl bg-[#55acce] min-w-[155px]">
              ...
              <h3 className="font-PlayfairDisplayBold capitalize text-white text-lg leading-5">
                {
                  bloodRequests?.filter(
                    (request) => request.reqStatus === "Accepted",
                  ).length
                }
              </h3>
            </div>
            <div className="py-3 px-4 rounded-xl bg-[#B3C100] min-w-[155px]">
              ...
              <h3 className="font-PlayfairDisplayBold capitalize text-white text-lg leading-5">
                {
                  bloodRequests?.filter(
                    (request) => request.reqStatus === "Pending",
                  ).length
                }
              </h3>
            </div>
            <div className="py-3 px-4 rounded-xl bg-[#AC3E31] min-w-[155px]">
              ...
              <h3 className="font-PlayfairDisplayBold capitalize text-white text-lg leading-5">
                {
                  bloodRequests?.filter(
                    (request) => request.reqStatus === "Rejected",
                  ).length
                }
              </h3>
            </div>
        ...
        {isFromDashboard && (
          <div className="w-full flex items-center justify-between mt-4">
            <p className="font-DMSansSemiBold text-slate-900 capitalize pb-4">
              List of Blood Requests
            </p>
            <Link
              className="flex items-center gap-x-2.5 cursor-pointer"
              href="/profile/bloodBank/requests"
            >
              <p className="text-[#C4473B] font-DMSansRegular text-sm">
                View All
              </p>
        ...
        <div
          className={cx(
            `bg-white w-full py-4 px-2.5 mt-4 rounded-lg`,
            { "!mt-0": isFromDashboard },
            [shadow.lightShadow],
          )}
        >
          {!isFromDashboard && (
              ...
              {bloodRequests && bloodRequests.length > 0 ? (
                (isFromDashboard
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell
                      className={cx("!grid !grid-cols-3 gap-x-1 gap-y-0.5", {
                        "!grid-cols-2": isFromDashboard,
                      })}
                    >
                      {request.receivedBlood.map((val: string, ind: number) => {
                        return (
                          <span
                            className="bg-slate-50 p-1 rounded-md text-center"
                            key={ind}
                          >
                            {val}
                          </span>
