...
const BBDashboardUsers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        users.sort((a, b) =>
          a.requests + a.donations > b.requests + b.donations ? -1 : 1,
        );
  ...
  return (
    <div className="flex justify-between w-full mt-7">
      ...
      <div
        className={`w-[26%] bg-white border border-slate-300 rounded-[10px] px-4 py-1`}
      >
        <p className="text-slate-500 font-DMSansMedium capitalize text-lg mb-2">
          Users
        </p>
        {usersData?.slice(0, 6).map((userData: any, index: number) => (
          <div
            key={index}
            className="flex items-center justify-between border-b border-slate-300 py-2"
          >
            <div className="flex items-center gap-x-5">
              ...
              <div className="flex flex-col gap-y-0.5">
                <p className="text-slate-500 font-DMSansMedium text-lg">
                  {userData?.user?.firstName + " " + userData?.user?.lastName}
                </p>
                <p className="text-slate-500 font-DMSansMedium text-sm">
                  Requests: {userData?.requests}, Donations:{" "}
                  {userData?.donations}
                </p>
              </div>
            </div>
            <Link
              ...
              className="bg-[#E3E3E3] p-2 rounded-md cursor-pointer"
            >
