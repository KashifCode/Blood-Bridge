...
const EditEvent = ({ eventToEdit, handleAddEvent }: { eventToEdit: any, handleAddEvent: () => void }) => {
    ...
    const handleNewEvent = (data: newEventInterface, e: any) => {
        ...
        let newEditedEvent = { ...allEvents?.filter((event) => event._id === eventToEdit._id)[0], ...data }

        axios.put(url, data, {
            withCredentials: true,
        })
            .then(res => {
                dispatch(updateAllEvents({ events: [...allEvents?.filter((event) => event._id !== eventToEdit._id), newEditedEvent] } as any))
    ...
    return (
        <div className='w-full px-[5%]'>
            <div className='flex justify-between items-center'>
                ...
                <Button className='bg-red-700 hover:bg-red-700 rounded-full text-white !h-auto !py-2 !px-5' onClick={() => handleAddEvent()}>Add Event</Button>
            </div>
            <form onSubmit={handleSubmit(handleNewEvent)} className="space-y-4">
                ...
                <div className='flex flex-col space-y-2'>
                    ...
                    <Input type='text' {...register('eventName')} defaultValue={eventToEdit?.eventName} />
                </div>
                ...
                <div className='flex flex-col space-y-2'>
                    ...
                    <Input type='date' className='bg-[#E9DFDF]' placeholder="Date" {...register("eventDate")}
                        defaultValue={new Date(eventToEdit?.eventDate)?.toISOString()?.substring(0, 10)} />
                </div>
                ...
                <div className='flex flex-col space-y-2'>
                    <Label className='!mb-2'>Image</Label>
                        ...
                        />
                        {image &&
                            <p className='text-sm text-black mt-2'><strong>Selected File:</strong> {image ? image?.[0]?.name : eventToEdit.image}</p>
                        }
                    </div>
