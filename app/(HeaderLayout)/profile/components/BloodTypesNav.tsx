    ...
    <div className="w-full flex flex-col mt-2.5 gap-y-5">
      ...
      <div className="flex items-center flex-wrap gap-x-5 gap-y-4 mt-2.5">
        {BloodTypes.map((type: any, index: number) => (
          <div
            key={index}
            className={`w-32 h-24 rounded-xl flex flex-col items-center justify-center gap-y-2.5`}
            style={{ backgroundColor: colors[index] }}
          >
            <p className="text-white text-2xl !leading-7 font-PlayfairDisplayBold text-center">
              {type.bloodGroup}
            </p>
            <div className="bg-[#D9D9D9] rounded px-5 py-0.5">
              <p className="text-black font-PlayfairDisplayBold text-center">
                {type.stock} Bags
              </p>
            </div>
          </div>
      ...
      <div className="w-max flex flex-col gap-y-5 mt-6 mb-3">
        <div
          ...
          onClick={() => handleRedirect("/profile/bloodBank/donation")}
        >
        ...
        <div
          className="w-full bg-[#E3E3E3] rounded-[10px] py-5 flex gap-x-32 items-center justify-between px-6 cursor-pointer"
          onClick={() => handleRedirect("/profile/bloodBank/requests")}
        >
          <div className="flex gap-x-3 items-center">
