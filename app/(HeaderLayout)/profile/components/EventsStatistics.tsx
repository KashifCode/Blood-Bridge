    ...
    <div className="w-full h-full pb-5">
      <div className="w-full flex gap-x-7 items-center">
        <div className="py-3 px-4 rounded-xl bg-[#AE3C60] min-w-[155px]">
          <h3 className="font-PlayfairDisplayBold capitalize text-white text-xl leading-5">
            Total Events
          </h3>
          <h3 className="font-PlayfairDisplayBold capitalize text-white text-xl leading-5">
            {allEvents?.length}
      ...
      <div className={`bg-white w-full flex gap-x-4 pb-4 px-2.5`}>
        <div className="w-[60%]">
          <Table
            className={`overflow-hidden !rounded-lg ${shadow.lightShadow}`}
          >
            <TableHeader>
              <TableRow>
              ...
              {allEvents &&
                allEvents.map((event, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="!grid !grid-col-1 gap-1">
                      {event?.guests?.map((val: string, ind: number) => {
                        return (
                          <span
                            className="bg-slate-200 bg-opacity-80 p-1 rounded-md text-center w-max"
                            key={ind}
                          >
                            {val}
                          </span>
                    ...
                    <TableCell
                      className="underline text-[#00832C] font-DMSansMedium text-center min-w-max cursor-pointer"
                      onClick={() => handleNotifyUsers(index)}
                    >
                      Notify Users
                    </TableCell>
                    <TableCell className="w-max">
                      <div className="flex flex-col gap-y-2 items-center justify-center">
                        <Button
                          className="bg-red-700 hover:bg-red-600 text-white font-DMSansMedium text-sm !h-auto !py-1 !px-3 rounded-md"
                          disabled={isEventEdit ? true : false}
