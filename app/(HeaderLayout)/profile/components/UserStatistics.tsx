        ...
        <div className='w-full'>
            <div className='w-full flex gap-x-7 items-center'>
                <div className='py-3 px-4 rounded-xl bg-[#20283E] min-w-[155px]'>
                    <h3 className='font-PlayfairDisplayBold capitalize text-white text-lg leading-5'>Total Users</h3>
                    <h3 className='font-PlayfairDisplayBold capitalize text-white text-lg leading-5'>{(bloodDonations && bloodRequests) && [...bloodDonations, ...bloodRequests].filter((v, i, a) => a.findIndex(t => (t.user?._id === v.user?._id)) === i)?.length}</h3>
                </div>
                <div className='py-3 px-4 rounded-xl bg-[#0A5620] min-w-[155px]'>
                    <h3 className='font-PlayfairDisplayBold capitalize text-white text-lg leading-5'>Total Donars</h3>
                    <h3 className='font-PlayfairDisplayBold capitalize text-white text-lg leading-5'>{(users?.donatedUsers)?.length}</h3>
                </div>
                <div className='py-3 px-4 rounded-xl bg-[#B3C100] min-w-[155px]'>
                    <h3 className='font-PlayfairDisplayBold capitalize text-white text-lg leading-5'>Total Requesters</h3>
                    <h3 className='font-PlayfairDisplayBold capitalize text-white text-lg leading-5'>{(users?.requestedUsers)?.length}</h3>
                </div>
            </div>

            <div className='w-full flex justify-end'>
                <div className='flex items-center gap-x-4'>
                    <p className='font-DMSansSemiBold text-slate-900 capitalize'>Filter</p>
                    <select
                ...
                <Table className='!rounded-lg overflow-hidden'>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className='!px-3 !py-1.5 bg-slate-50 !border !border-black !border-opacity-10 !h-auto !font-DMSansMedium'>Blood Group</TableHead>
                            ...
                            {(filter === 'all' || filter === 'requester') &&
                                <TableHead className='!px-3 !py-1.5 bg-slate-50 !border !border-black !border-opacity-10 !h-auto !font-DMSansMedium'>Total Requests</TableHead>
                            }
                            {(filter === 'all' || filter === 'donar') &&
                                <TableHead className='!px-3 !py-1.5 bg-slate-50 !border !border-black !border-opacity-10 !h-auto !font-DMSansMedium'>Total Donations</TableHead>
                            }
                            {filter === 'donar' &&
                    ...
                    <TableBody className=''>
                        {filter === 'all' && users?.users && users.users.map((aUser: any, index: number) => (
                            <TableRow key={index}>
                                <TableCell className="text-center">{index + 1}</TableCell>
                                <TableCell>{aUser?.user?.name}</TableCell>
                                <TableCell className='text-center'>{aUser?.user?.user ? calculateAge(new Date(aUser?.user?.user?.dob.split('T')[0])) : '-'}</TableCell>
                                <TableCell>{aUser?.user?.contact}</TableCell>
                                <TableCell className='text-center'>{aUser?.user?.user?.bloodGroup}</TableCell>
                                <TableCell>{(aUser?.donations > 0 && aUser?.requests > 0) ? 'Donor/Requester' : (aUser?.donations > 0 && aUser?.requests === 0) ? 'Blood Donar' : (aUser?.donations === 0 && aUser?.requests > 0) ? 'Blood Requester' : '-'}</TableCell>
                                <TableCell className='text-center'>{aUser?.requests}</TableCell>
                                <TableCell className='text-center'>{aUser?.donations}</TableCell>
                            </TableRow>
                        ))}
                        {filter === 'donar' && users?.donatedUsers && users.donatedUsers.map((aUser: any, index: number) => (
                            <TableRow key={index}>
                                <TableCell className="text-center">{index + 1}</TableCell>
                                <TableCell>{aUser?.user?.name}</TableCell>
                                <TableCell className='text-center'>{aUser?.user?.user ? calculateAge(new Date(aUser?.user?.user?.dob.split('T')[0])) : '-'}</TableCell>
                                ...
                                <TableCell>{(aUser?.donations > 0 && aUser?.requests > 0) ? 'Donor/Requester' : (aUser?.donations > 0 && aUser?.requests === 0) ? 'Blood Donar' : (aUser?.donations === 0 && aUser?.requests > 0) ? 'Blood Requester' : '-'}</TableCell>
                                <TableCell className='text-center'>{aUser?.donations}</TableCell>
                                <TableCell className='text-center'>{aUser?.user?.disease}</TableCell>
                            </TableRow>
                        ))}
                        {filter === 'requester' && users?.requestedUsers && users.requestedUsers.map((aUser: any, index: number) => (
                            <TableRow key={index}>
                                <TableCell className="text-center">{index + 1}</TableCell>
                                <TableCell>{aUser?.user?.name}</TableCell>
                                <TableCell className='text-center'>{aUser?.user?.user ? calculateAge(new Date(aUser?.user?.user?.dob.split('T')[0])) : '-'}</TableCell>
