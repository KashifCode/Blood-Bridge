        ...
        >
          ...
          {bloodBanks.map((BB_Data, index) => (
            <Marker
              key={index}
              position={{
                lat: BB_Data?.bloodBank?.location?.coordinates?.[1],
                lng: BB_Data?.bloodBank?.location?.coordinates?.[0],
              }}
              icon={
                "https://uploadthing.com/f/f800f3eb-bdde-4a93-ae9d-7ff4cef35f02-dth3ms.png"
              }
              onClick={() => {
                map.setCenter({
                  lat: BB_Data?.bloodBank?.location?.coordinates?.[1],
                ...
                >
                  <div className="flex flex-col gap-y-1">
                    <div className="flex items-center gap-x-1">
                      <h3 className="text-red-800 text-xs font-bold font-LatoBold">
                        Name:{" "}
                      </h3>
                      <p className="text-black text-xs font-LatoRegular">
                        {BB_Data?.bloodBank?.name}
                      </p>
                    </div>
                    <div className="flex items-center gap-x-1">
                      <h3 className="text-red-800 text-xs font-bold font-LatoBold">
                    ...
                    <h3 className="text-red-800 text-xs font-LatoBold">
                      Blood Groups:{" "}
                    </h3>
                    {BB_Data?.bloodGroups === null && (
                      <p className="text-black text-xs font-LatoRegular">
                        No Blood Types in Stock
                      </p>
                    )}
                    <div className="grid grid-cols-4 gap-y-2 mb-2">
                      {BB_Data?.bloodGroups?.map((BG_Data: any, index: any) => (
                        <div
                          key={index}
                          className={cx(
                            "flex flex-col items-center justify-between w-full bg-white py-0.5 border-r border-red-800",
                            {
                              "!border-none":
                        ...
                        >
                          <div>
                            <p className="font-LatoBold text-black text-xs">
                              {BG_Data.bloodGroup}
                            </p>
                          </div>
                          <p className=" font-LatoRegular text-black text-xs">
                            {BG_Data.stock}
                          </p>
                        </div>
                      ))}
