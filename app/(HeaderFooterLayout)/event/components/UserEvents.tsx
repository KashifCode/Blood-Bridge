  ...
  const handleMonthChange = (newCurrentMonth: string) => {
    if (newCurrentMonth === "all") {
      ...
      setFilteredEvents(
        events.filter(
          (event) =>
            new Date(event?.eventDate?.substring(0, 10))
              ?.toLocaleString("default", { month: "short" })
              .toLowerCase() === newCurrentMonth,
        ),
      );
    }
  };
  ...
  return (
    <div className="w-full flex flex-col pl-[6%] pr-[3%] mt-12 2xl:!pr-[6%] 2xl:!justify-between mb-6 md:!mb-1 items-center gap-y-6">
      <div className="w-full flex justify-between items-center">
        ...
        <select
          className="bg-red-700 outline-0 border-none text-[#E9DFDF] rounded-md py-1 px-2"
      ...
      <div className={cx("grid grid-cols-1 gap-x-8 gap-y-6", {'!grid-cols-1 sm:!grid-cols-2 lg:!grid-cols-3 2xl:!grid-cols-4': filteredEvents.length !== 0})}>
        {filteredEvents?.map((event, index) => (
          <div
            key={index}
            className={`w-full flex flex-col ${shadow.lightShadow}`}
          >
            <div className="w-full">
              <Image
                ...
                width={300}
                height={212}
                className="!w-full !min-h-[212px] !object-cover rounded-t-lg"
              />
            </div>
            <div className={`w-full flex bg-white rounded-lg p-4 mb-4`}>
              <div className="w-1/5 flex flex-col justify-center items-center">
                <p className="font-DMSansMedium text-base text-black">
                  {new Date(event?.eventDate?.substring(0, 10))?.toLocaleString(
                    "default",
                    { month: "short" },
                  )}
                </p>
                <p className="font-DMSansMedium text-base text-black">
                  {new Date(event?.eventDate?.substring(0, 10))?.getDate()}
                </p>
              </div>
              <div className="w-4/5 flex flex-col gap-y-1">
                <p className="font-LatoMedium text-lg text-black">
                  {event?.eventName}
                </p>
                <p className="font-LatoRegular text-sm text-black">
                  {event?.description}
                </p>
                <div className="flex items-center gap-x-1">
                  <MapPin size={20} />
                  <p className="font-LatoRegular text-base text-black">
                    {event?.venue}
                  </p>
                </div>
                <div className="flex items-center gap-x-1">
