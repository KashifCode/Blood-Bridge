...
const RequestDonateBlood = ({
  bloodBankId,
  callType,
}: RequestDonateBloodProps) => {
  ...
  const submitForm = (data: RequestBloodProps) => {
    if (isRequest) {
      ...
      const bloodGroup = bloodBankOpen?.bloodGroups.find(
        (bloodGroup: any) => bloodGroup.bloodGroup === data.bloodGroup,
      );
      if (bloodGroup?.stock < data?.bloodBags!) {
        toast.error(
          "Requested Blood Bags are not available, please select less quantity",
        );
        return;
      }
          ...
          <div className="w-full flex flex-col md:!flex-row items-start gap-y-8 sm:!gap-y-0 sm:!gap-x-[9%]">
            <div className="w-full">
              ...
              >
                ...
                <div className="flex flex-col gap-y-1">
                  ...
                  >
                    <option value="">
                      {isRequest ? "Select Blood Group" : "your blood type"}
                    </option>
                    {bloodBankOpen?.bloodGroups.map(
                      (BG_Data: any, index: number) => (
                        <option key={index} value={BG_Data.bloodGroup}>
                          {BG_Data.bloodGroup}
                        </option>
                      ),
                    )}
                ...
                <div className="flex flex-col gap-y-1">
                  <Label className="!font-LatoMedium font-medium pl-1">
                    {isRequest ? "Blood Needed On" : "Donate On (Availability)"}
                  </Label>
                  <select
                    className="w-full focus:outline-0 outline-0 rounded-2xl bg-stone-200 py-1.5 px-3"
                  ...
                  >
                    ...
                    <option value={`${tomorrow.toLocaleDateString()}`}>
                      {tomorrow.toDateString()}
                    </option>
                    <option value={`${dayAfterTomorrow.toLocaleDateString()}`}>
                  ...
                  <div className="grid grid-cols-3 gap-y-2 gap-x-2.5">
                    {
                      ...
                      Array.from({ length: bloodBags }, () => 0).map(
                        (_, index) => (
                          <div key={index} className="flex flex-col gap-y-1.5">
                            <Label className="!font-LatoMedium font-medium pl-1">
                              Exchange Type {index + 1}
                            </Label>
                            <select
                              className="w-full focus:outline-0 outline-0 rounded-2xl bg-stone-200 py-1.5 px-3"
                              {...register(`receivedBlood.${index}`)}
                            >
                              <option value="">Select</option>
                              {Constants.bloodGroups.map(
                                (bloodGroup: string, index: number) => (
                ...
                <div>
                  ...
                  >
                    {isRequest ? "Place Request" : "Donate"}
                  </Button>
                </div>
              </form>
            </div>
            <div className="w-full">
              <h1 className="text-[28px] font-LatoBold !font-semibold">
                {isRequest ? "Need Blood Urgently?" : "Donate Blood Toady!"}
              </h1>
              ...
              {!isRequest ? (
                <p className="font-LatoRegular">
                  If you are looking to donate blood, we are here to help you.
                  To address urgent blood requirements, please reach out to us
                  by dialing our Unified Access Number (UAN). Alternatively, you
                  can request a callback from our team to swiftly connect with
                  you on your blood donation call. Your well-being is our
                  priority, and we believe that by donating your blood you can
                  remain in good health.
                </p>
              ) : (
                <p className="font-LatoRegular">
                  If you find yourself in a critical need for blood, we are here
                  to offer our support. To address urgent blood requirements,
                  please reach out to us by dialing our Unified Access Number
                  (UAN). Alternatively, you can request a callback from our team
                  to swiftly connect you with the assistance you require during
                  emergencies. Your well-being is our priority, and we are
                  committed to being there for you when you need it the most.
                </p>
              )}
