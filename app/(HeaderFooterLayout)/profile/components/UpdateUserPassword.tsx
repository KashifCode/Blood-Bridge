...
const UpdateUserPassword = ({
  ...
  const [typedPasswordStrength, setTypedPasswordStrength] =
    useState<string>("");
  const schema: ZodType<UpdateUserData> = z
    .object({
      oldPassword: z
        .string()
        .nonempty({ message: "Current Password must not be empty" }),
      newPassword: z
        .string()
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        <label
          className="text-black font-RobotoMedium mb-0.5 ps-3 capitalize"
          htmlFor="oldPassword"
        >
          Current Password
        </label>
        <input
          ...
          className={cx(
            "bg-white rounded-3xl w-full py-2 px-3.5 focus:outline-0",
            { "!rounded-[5px]": isForBloodBank },
          )}
          type="password"
          readOnly={!changeDetails ? true : false}
        />
      </div>
      ...
      <div className="relative mb-5">
        <div className="w-full flex flex-col">
          ...
          <input
            ...
            className={cx(
              "bg-white rounded-3xl w-full py-2 px-3.5 focus:outline-0 text-red-500",
        ...
        <div
          className="absolute bottom-3 my-auto right-3 cursor-pointer"
        ...
        {typedPasswordStrength !== "" && (
          <div className="absolute left-2 -bottom-6">
            <p className="pl-1.5 text-zinc-500 text-sm font-LatoRegular tracking-[1px]">
              Password is{" "}
              <span
                className={cx("text-green-600 uppercase text-xs", {
      ...
      {changeDetails && (
        <>
          {isForBloodBank ? (
            <Button
              type="submit"
              className="!w-max !rounded-3xl !bg-darkRed hover:!bg-red-800 mt-1 !h-auto !py-1.5 min-w-[140px] !text-sm"
            >
              Change Password
            </Button>
          ) : (
            <Button
              type="submit"
              className="!w-max !rounded-3xl !bg-darkRed hover:!bg-red-800 mt-1 !h-auto !py-2 min-w-[140px] !text-base"
            >
              Change Password
            </Button>
          )}
